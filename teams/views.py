...
def team_ladder(request):
    active_teams = Team.objects.filter(disbanded=False).order_by('-rating')
    return render(request, 'team_ladder.html', {'active_teams': active_teams})
...
def send_invitation(request, team_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if invited_user in team.players.all():
                ...
                return redirect('user_is_teammate')

            # Check if an invitation already exists for the same team and invited user
            existing_invitation = TeamInvitation.objects.filter(team=team, invited_user=invited_user).first()
            if existing_invitation:
                # Redirect to a page informing the user that an invitation already exists
                return redirect('invitation_already_sent')

            # Create and save the new invitation
            invitation = TeamInvitation(team=team, inviting_user=request.user, invited_user=invited_user)
            invitation.save()
    ...
    return render(request, 'send_invitation.html', {'team': team, 'form': form})
...
def accept_invitation(request, invitation_id):
    ...
    if not invitation.is_accepted:
        ...
        invitation.invited_user.current_team = invitation.team
...
def deny_invitation(request, invitation_id):
    invitation = get_object_or_404(TeamInvitation, id=invitation_id)
    ...
    invitation.delete()
    return redirect('pending_team_invites')


def user_is_teammate(request):
    return render(request, 'user_is_teammate.html')

def invitation_already_sent(request):
    return render(request, 'invitation_already_sent.html')

def invitation_sent(request):
    return render(request, 'invitation_sent.html')


def pending_team_invites(request):
    ...
    if is_verified:
        ...
        return render(request, 'pending_team_invites.html', {'pending_team_invites': pending_team_invites})
    else:
        return redirect('request_verification')        
...
def transfer_ownership(request, team_id, new_owner_id):
    team = get_object_or_404(Team, id=team_id)
