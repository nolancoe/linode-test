...
class DirectChallenge(models.Model):
    challenging_team = models.ForeignKey(Team, on_delete=models.CASCADE, related_name='direct_challenges')
    challenged_team = models.ForeignKey(Team, on_delete=models.CASCADE, related_name='challenged_direct_challenges')
    date_created = models.DateTimeField(default=timezone.now)
    scheduled_date = models.DateTimeField(null=True, blank=True)
    accepted = models.BooleanField(default=False)
    search_only = models.BooleanField(default=False)
...
class DisputeProof(models.Model):
    match = models.ForeignKey(Match, on_delete=models.CASCADE)
    owner = models.ForeignKey(Profile, on_delete=models.CASCADE)
    claim = models.TextField(blank=True, null=True)
    game1_screenshot = models.ImageField(upload_to='dispute_screenshots/', blank=True, null=True)
    game2_screenshot = models.ImageField(upload_to='dispute_screenshots/', blank=True, null=True)
...
class Dispute(models.Model):
    match = models.ForeignKey(Match, on_delete=models.CASCADE)
    team1_owner_proof = models.OneToOneField(DisputeProof, on_delete=models.CASCADE, related_name='team1_owner_dispute', null=True)
    team2_owner_proof = models.OneToOneField(DisputeProof, on_delete=models.CASCADE, related_name='team2_owner_dispute', null=True)
    created_at = models.DateTimeField(default=timezone.now)
